...
class DARTSScheduledDynamicOptimizer(DARTSOptimizer):
    """
    ...
    def __init__(
        ...
        """
        super(DARTSScheduledDynamicOptimizer, self).__init__(config, op_optimizer, arch_optimizer, loss_criteria)
        self.epochs = config.search.epochs
        self.k = 1
        self.loss_threshold = 3

    @staticmethod
    def sample_alphas(edge, epoch, max_epochs, k):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # arch_parameters = torch.unsqueeze(edge.data.alpha, dim=0)

        k = min(k, len(edge.data.alpha))
    ...
    def step(self, data_train, data_val, epoch):
        ...
        self.graph.update_edges(
            update_func=lambda edge: self.sample_alphas(edge, epoch, self.epochs, self.k),
            scope=self.scope,
        ...
        if train_loss.item() < self.loss_threshold:
            self.loss_threshold = train_loss.item()
            self.k += 1
